  ...
  while ((match = trRegex.exec(html)) !== null) {
    ...
    if (trContent.includes('class="latcpb"') || trContent.includes('class=latcpb')) {
      ...
      if (tds.length >= 5) {
        ...
        // Extract vacancies and clean title
        let title = postName;
        let vacancies = 45;
...
// 2. Parsing Exams (Admit Cards / Results / Answer Keys)
function parseExams(filePath, isJharkhandPage = false) {
  ...
  while ((match = aRegex.exec(html)) !== null) {
    ...
    if (isAdmitCard || isResult || isAnswerKey) {
      ...
      // clean title: e.g. "JSSC Excise Constable Admit Card 2026 (Out)" -> "Excise Constable"
      let title = text
...
const main = async () => {
  ...
  const allIndiaJobs = parseJobs(file1972, false);
  const jharkhandJobs = parseJobs(file1984, true);
  const allJobs = [...jharkhandJobs, ...allIndiaJobs];
  
  // Dedup jobs by URL/applyLink
  const uniqueJobsMap = new Map();
  for (const job of allJobs) {
    uniqueJobsMap.set(job.applyLink, job);
  }
  const uniqueJobs = Array.from(uniqueJobsMap.values());
  console.log(`Extracted ${uniqueJobs.length} unique jobs.`);
  
  const allIndiaExams = parseExams(file1972, false);
  const jharkhandExams = parseExams(file1984, true);
  ...
  console.log(`Extracted ${uniqueExams.length} unique exam notices.`);
  
  // Save to JSON
  const scrapedData = { jobs: uniqueJobs, exams: uniqueExams };
  const scrapedPath = path.join('config', 'scraped_data.json');
  fs.writeFileSync(scrapedPath, JSON.stringify(scrapedData, null, 2), 'utf-8');
  console.log(`Saved scraped data to ${scrapedPath}`);
  
  // Insert directly into PostgreSQL database if DATABASE_URL is set
  if (process.env.DATABASE_URL) {
    console.log('Connecting to PostgreSQL database to upsert scraped records...');
    ...
    try {
      ...
      // Upsert Exams
      console.log('Upserting exams/notices...');
      ...
      console.log('✅ Exams/notices upserted successfully.');
      
    } catch (err) {
      console.error('Error executing upserts on PostgreSQL:', err.message);
    } finally {
      await pool.end();
    }
  } else {
    console.log('DATABASE_URL is not set. Skipping PostgreSQL upserts.');
  }
};
